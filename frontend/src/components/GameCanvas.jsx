...
export default function GameCanvas({
  ...
  useEffect(() => {
    ...
    Object.entries(serverState).forEach(([id, state]) => {
      if (id === playerId) {
        ...
        // For now, we trust client-side prediction
        return;
      }
  ...
  const render = useCallback(
    (ctx, w, h) => {
      ...
      Object.entries(otherPlayersRef.current).forEach(([id, playerData]) => {
        drawCar(ctx, playerData.current, '#ff6b6b', id);
      });

      // Draw local player
      drawCar(ctx, localPlayerRef.current, '#00ff00', playerId);
  ...
  const drawOvalTrack = (ctx) => {
    ...
    // Center line (dashed racing line)
